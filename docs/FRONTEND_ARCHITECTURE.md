...
**Idempotency headers** (`API_SPEC.md` §1/§3 — `Idempotency-Key` on task creation, submit, review, mark-billed, issue resolution): generated client-side per request (a UUID) and attached in the API client wrapper, not per-feature — one place, matching the "single client instance" pattern above.
