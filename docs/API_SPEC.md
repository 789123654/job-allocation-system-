...
| Method & path | Role | Idempotency pattern | Notes |
|---|---|---|---|
| `GET /issues/{id}` | Owner only | — (read) | **Added 2026-09-12** (Phase 4, Owner-side Tasks slice) — the Dashboard's "Issues Raised" panel (PRD §2.4) needs the issue's actual description text; `GET /notifications` only carries `{type, task_id, issue_id}`, and `resolve_issue` was the only prior way to touch an `Issue` at all, a mutating read wrong for a display-only panel. Same RLS/Owner-only shape as `resolve_issue` below, not a new authorization mechanism |
