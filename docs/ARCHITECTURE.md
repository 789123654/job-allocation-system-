...
3. A Postgres trigger on `auth.users` (`on_auth_user_created`, the pattern Supabase's own docs
   ...
   the matching `profiles` row — not a second manual insert from application code. **Fires on `UPDATE`
   as well as `INSERT`** (migration `8d533906f6cd`, fixing a real bug in the finding-#11 migration the
   same day it landed): GoTrue applies `app_metadata` via a follow-up `UPDATE` *after* the initial insert,
   not as part of it — confirmed empirically when this broke every real admin-provisioned user in CI, and
   by Supabase's own community troubleshooting material ("App metadata is updated after creating a user
   row in the database, so you can't listen to the user insert call to update app metadata"). The trigger
   guards on `raw_app_meta_data->>'firm_id' IS NOT NULL` and only fires the actual insert once that's
   true, `ON CONFLICT DO NOTHING` making it safe to run again on later unrelated updates. Still happens
   within the single `createUser()` call from the caller's perspective — by the time step 2's call
   returns, the row already exists, which is what step 4 below actually depends on.
