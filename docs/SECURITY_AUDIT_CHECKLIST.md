...
phase: Phase 5 (Hardening) — Observability Phase 1, independent-review fixes, BATCH 1 (pilot of VERIFICATION_WORKFLOW.md). Four confirmed leaks reproduced by the author after the 2026-09-19 review: (1) the redactor stops at a blank line inside a DETAIL/CONTEXT value; (2) ExceptionGroup `    | `-prefixed tracebacks bypass it; (3) `_ECHOED_QUOTED_VALUE` is quadratic (16 KB = 17.7 s) and runs before the size clip; (4) the frontend breadcrumb regex misses attribute values containing a double quote. Batches 2-3 (remaining review findings) are separate passes.
scope_files: backend/app/core/redaction.py, backend/app/core/logging_setup.py, frontend/src/lib/sentry-context.ts; new tests backend/tests/core/test_redaction_attack.py, backend/tests/core/attack_inputs.py, frontend/src/lib/sentry-context.attack.test.ts
date: 2026-09-20
...
- `blind_test`: yes — data-boundary code (scrubbers). Author: a fresh Sonnet agent in a HEAD worktree where the implementation does not exist, given only the contract, public signatures, owasp-wstg chapters 07 and 08 and the requirement rows below. It is deliberately NOT told the four known bug shapes, so what it finds on its own calibrates the pilot. The fixer does not read the delivered tests until the code is written.
- `hostile_inputs_tested`: PENDING — to be pasted from the blind author's delivered files (real Postgres error text with hostile values, ExceptionGroup wrapping, size/scaling bound).
- `req_01`: regexes over untrusted text are free of exponential/quadratic backtracking, anchored to the whole input, use no `.` wildcard, and input length is bounded | ASVS 1.3.12 (owasp-asvs-5/chapters/v1-encoding-sanitization.md:52, L3); Input_Validation_Cheat_Sheet.md:33,75 | applies: redaction.py `_ECHOED_QUOTED_VALUE`, sentry-context.ts `ATTRIBUTE_VALUES` | evidence: PENDING — scaling test (doubling the input must not much more than double the time) and a 256 KB bound, backend and frontend.
- `req_02`: sensitive data must not reach logs or events | TCASVS 3.2.3 (owasp-tcasvs/chapters/v3-data-storage-protection.md:25); ASVS 16.2.5 (owasp-asvs-5/chapters/v16-security-logging-error-handling.md:27); Logging_Cheat_Sheet.md:188; Multi_Tenant_Security_Cheat_Sheet.md:979 | applies | evidence: PENDING — end-to-end marker tests: real Postgres errors through the log formatter and the Sentry scrubber leave no marker.
- `req_03`: exceptions are handled without leaking sensitive information, including ExceptionGroup members and chained causes | TCASVS 4.6.1 (owasp-tcasvs/chapters/v4-code-quality-exploit-mitigation.md:67) | applies | evidence: PENDING — ExceptionGroup and chained-cause marker tests.
- `req_04`: a log record stays one valid JSON line whatever the message contains (log injection) | Logging_Cheat_Sheet.md:232 | applies, as a regression check | evidence: PENDING — parse every formatted line with json.loads.
- `req_05`: the API consumer gets a generic error | ASVS 16.5.1 (owasp-asvs-5/chapters/v16-security-logging-error-handling.md:55) | N/A: no route or handler code is in this batch; re-verified on the final diff | evidence: PENDING — end-of-work diff shows no change under backend/app/api/.
- `req_06`: attack-intent tests written independently, before the fix | owasp-wstg chapters 07-input-validation.md and 08-error-handling.md; skill-verification-discipline.md failure mode 10.2 | applies | evidence: PENDING — delivered test files, then negative controls (mutants) run against them.
