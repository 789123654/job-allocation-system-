...
def test_owner_poll_generates_the_assignees_own_task_notifications(session: Session) -> None:
    """Firm-wide scan (_scan_firm_deadlines): the assignee's own-task deadline notifications are
    generated by *anyone's* poll — the employee no longer has to open the app first.
    """
    owner = _owner(session)
    employee = _employee(session)
    _task_with_deadline(session, owner, employee, datetime.now(UTC) - timedelta(days=1))  # overdue

    # The owner polls — the employee has not.
    crud.list_notifications(session, owner, 0, 20, unread_only=False)

    assert [n.type for n in _notifications_for(session, employee.id)] == ["task_overdue_own"]
